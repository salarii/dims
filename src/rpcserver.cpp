...
void StartRPCThreads()
{
    ...
    if (fUseSSL)
    {
	  ...
	  if (!pathCertFile.is_complete()) pathCertFile = filesystem::path(GetDataDir(common::AppType::Tracker)) / pathCertFile;
        if (filesystem::exists(pathCertFile)) rpc_ssl_context->use_certificate_chain_file(pathCertFile.string());
        else LogPrintf("ThreadRPCServer ERROR: missing server certificate file %s\n", pathCertFile.string());

        filesystem::path pathPKFile(GetArg("-rpcsslprivatekeyfile", "server.pem"));
        if (!pathPKFile.is_complete()) pathPKFile = filesystem::path(GetDataDir(common::AppType::Tracker)) / pathPKFile;
	...
	for (int i = 0; i < GetArg("-rpcthreads", 1); i++)
