...
public:
    ...
    void refreshAddressTable()
    {
        cachedAddressTable.clear();
        {
            ...
            {
                const CMnemonicAddress& address = item.first;
			...
			bitcoinAddress.GetKeyID(keyID);
				...
				//common::CPeriodicActionExecutor< client::ClientResponses >::getInstance()->addAction( new client::CSendBalanceInfoAction( address.ToString() ), 6000 );
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        CTxDestination curAddress = CMnemonicAddress(rec->address.toStdString()).Get();
        if(index.column() == Label)
        {
            ...
            CTxDestination newAddress = CMnemonicAddress(value.toString().toStdString()).Get();
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address)
{
    ...
    if(type == Send)
    {
        ...
        {
            LOCK(wallet->cs_wallet);
            if(wallet->mapAddressBook.count(CMnemonicAddress(strAddress).Get()))
            {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
    ...
    else if(type == Receive)
    {
        ...
        strAddress = CMnemonicAddress(newKey.GetID()).ToString();
    }
    else
    {
        return QString();
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBook(CMnemonicAddress(strAddress).Get(), strLabel,
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->DelAddressBook(CMnemonicAddress(rec->address.toStdString()).Get());
    }
    return true;
}
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {
        LOCK(wallet->cs_wallet);
        CMnemonicAddress address_parsed(address.toStdString());
